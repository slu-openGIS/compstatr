...
These functions are only available on GitHub right now. In order to enable them, there is one breaking change for users - all variable names are standardize into `snake_case` at import. This will impact code that has been written using a prior version of `compstatr`.
