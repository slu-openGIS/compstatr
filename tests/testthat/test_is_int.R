...
test_that("input error triggered - no .data",
          expect_error(cs_is_int(var = ILEADSStreet, newVar = new_column),
                       "A existing data frame with data to be separated must be specified for .data"))
test_that("input error triggered - no variable specified",
          expect_error(cs_is_int(test_data, newVar = intersection),
