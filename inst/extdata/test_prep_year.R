...
## write files
cs_example(path = paste0(tmpdir, "/data2/"))

test_that("correctly specified functions execute without error", {
  expect_error(cs_prep_year(path = paste0(tmpdir,"/data2/")), NA)
})
...
## create temp directory with data subdir
tmpdir <- tempdir()
fs::dir_create(paste0(tmpdir,"/data3/"))

## write files
cs_example(path = paste0(tmpdir, "/data3/"))

## create results
results <- cs_prep_year(path = paste0(tmpdir,"/data3/"), verbose = TRUE)
...
test_that("incorrect parameters trigger error", {
  expect_error(cs_prep_year(path = paste0(tmpdir,"/data3/"), verbose = "ham"),
...
## create temp directory with data subdir
tmpdir <- tempdir()
fs::dir_create(paste0(tmpdir,"/data4/"))

## write files
cs_example(path = paste0(tmpdir, "/data4/"))

## add additional files
readr::write_csv(dec17, path = paste0(tmpdir,"/data4/december2018.CSV.html"))

test_that("too many files trigger error", {
  expect_error(cs_prep_year(path = paste0(tmpdir,"/data4/")),
               "There are too many files in the specified folder. Edit crime files in yearly batches of 12 monthly files.")
})

## delete files
fs::file_delete(paste0(tmpdir,"/data4/december2017.CSV.html"))
fs::file_delete(paste0(tmpdir,"/data4/december2018.CSV.html"))

test_that("too few files triggers warning", {
  expect_warning(cs_prep_year(path = paste0(tmpdir,"/data4/")),
